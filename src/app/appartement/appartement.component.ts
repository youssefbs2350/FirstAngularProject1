import { Component } from '@angular/core';
import { Residence } from '../core/models/residence';
import { Apartment } from '../core/models/appartement';
import { NgIf } from '@angular/common';
import { ActivatedRoute } from '@angular/router';

@Component({
  selector: 'app-appartement',
  templateUrl: './appartement.component.html',
  styleUrls: ['./appartement.component.css']
})
export class AppartementComponent {
  listResidences: Residence[] = [
    { id: 1, "name": "El fel", "address": "Borj Cedria", "image": "../../assets/images/R1.jpeg" },
    { id: 2, "name": "El yasmine", "address": "Ezzahra", "image": "../../assets/images/R2.jpg" },
    { id: 3, "name": "El Arij", "address": "Rades", "image": "../../assets/images/R3.jpg" },
    { id: 4, "name": "El Anber", "address": "Manzah 5", "image": "../../assets/images/R4.jpg" }
  ];
  listApartments:Apartment[]=[
    {id:1 , "appartNum" :1 , "floorNum" : 0 , "surface":100,"terrace":"oui" ,"surfaceTerrace":20,"category":"S+1","description":"AppartementS+1","residence":this.listResidences[0] },
    {id:2,"appartNum":2,"floorNum":0,"surface":130,"terrace":"non","surfaceTerrace":0,"category":"S+2","description":"AppartementS+2","residence":this.listResidences[0] }, 
    {id:3,"appartNum":3,"floorNum":8,"surface":140,"terrace":"non","surfaceTerrace":0,"category":"S+5","description":"AppartementS+5","residence":this.listResidences[1] }]
    
    
      list!:Apartment[];
      listfavoris:any[]=[];
      id!: number ;
appart!: Apartment ;
    checkappart(id:number) {
      this.list=this.listApartments.filter((appart)=>appart.residence.id==id) ;

    }
    likeappart(i: number) {
      const selectedAppart = this.listApartments[i];
    
      
      const isAlreadyLiked = this.listfavoris.some(appart => appart.id === selectedAppart.id);
    
      if (!isAlreadyLiked) {
        
        selectedAppart.id = selectedAppart.appartNum;
    
      
        this.listfavoris.push(selectedAppart);
        console.log(this.listfavoris);
    
        
        const residenceIndex = this.listResidences.findIndex(residence => residence.id === selectedAppart.residence.id);
        if (residenceIndex !== -1) {
          this.listResidences[residenceIndex].id = selectedAppart.id;
        }
      } else {
       
        console.log("This apartment is already in the favorites list.");
      }
    }
    likeAppart(appart: Apartment) {
      if (!this.listfavoris.includes(appart)) {
        this.listfavoris.push(appart);
        console.log(this.listfavoris);
      }
    }
    constructor (private route:ActivatedRoute){}
  
    ngOnInit(){
      this.id=this.route.snapshot.params['id'];
      this.list=this.listApartments.filter((appart)=>appart.residence.id==this.id);
      
      }
}
